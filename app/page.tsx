...
export default function Home() {
  ...
  return <main>
    {!ageOk && <div className="age-gate" role="dialog" aria-modal="true" aria-labelledby="age-title"><div className="age-card"><img src="/hoperarios-wordmark-v2.png" alt="Hoperários Cervejaria"/><p className="kicker">Conteúdo para maiores</p><h2 id="age-title">Você tem mais de 18 anos?</h2><p>Este site contém informações sobre bebidas alcoólicas.</p><div className="age-actions"><button autoFocus onClick={() => setAgeOk(true)}>Sim, tenho 18 anos ou mais</button><a href="https://www.google.com">Não, quero sair</a></div><small>Beba com moderação. Venda proibida para menores de 18 anos.</small></div></div>}

    <div className="page-progress" aria-hidden="true"><i style={{ height: `${progress * 100}%` }}></i></div>
    <section className="hero" id="top">
      <nav className="capsule-nav"><a className="logo-image" href="#top"><img src="/hoperarios-wordmark-v2.png" alt="Hoperários Cervejaria"/></a><div className="nav-links"><a href="#chopes">Chopes</a><a href="#momentos">Momentos</a><a href="#calculadora">Calculadora</a><a className="nav-cta" href={baseWa} target="_blank">Reservar barril ↗</a></div></nav>
    ...
    <section className="care reveal"><header><p className="kicker">Sem perrengue na festa</p><h2>Você brinda.<br/><i>A gente cuida.</i></h2><p>Não é só o barril. É a tranquilidade de receber o sistema completo, regulado e pronto para o primeiro copo.</p></header><div className="care-grid">{[["01","Cerveja da fonte","Marca própria da cervejaria. Não somos revenda."],["02","No ponto certo","Chopeira regulada para chopp gelado, cremoso e sem excesso de espuma."],["03","Gás garantido","Cilindro reserva incluso para o chopp não parar no melhor da festa."],["04","Gente daqui","Atendimento próximo em Volta Redonda e região, até 23h."]].map(x => <article key={x[0]}><b>{x[0]}</b><div className="care-icon"><i></i></div><h3>{x[1]}</h3><p>{x[2]}</p></article>)}</div></section>
